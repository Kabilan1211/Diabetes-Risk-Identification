...
model = tf.keras.Sequential([
    tf.keras.layers.Input(shape=(8,)),  # Input layer with 3 features (temperature, humidity, moisture)
...
dia = SVC(kernel='linear')
dia.fit(X_train, y_train)

# Save the SVM model to a file using pickle
with open('dia.pkl', 'wb') as model_file:
    pickle.dump(dia, model_file)
